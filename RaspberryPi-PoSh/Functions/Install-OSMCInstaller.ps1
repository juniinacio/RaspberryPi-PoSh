...
function Install-OSMCInstaller {
    ...
    process {
        ...
        try {
            ...
            $preseed = [Preseed]::New($destination)

            if (-not $USB.IsPresent) {
                $preseed.SetTargetSetting('storage', [PreseedOptionType]::PRESEED_STRING, 'sd')
            } else {
                $preseed.SetTargetSetting('storage', [PreseedOptionType]::PRESEED_STRING, 'usb')
            }

            if ($PSCmdlet.ParameterSetName -eq 'eth') {
                $preseed.SetNetworkSetting('interface', [PreseedOptionType]::PRESEED_STRING, 'eth')
            } else {
                $preseed.SetNetworkSetting('interface', [PreseedOptionType]::PRESEED_STRING, 'wlan')
                $preseed.SetNetworkSetting('ssid', [PreseedOptionType]::PRESEED_STRING, $SSID)
                $preseed.SetNetworkSetting('wlan_key', [PreseedOptionType]::PRESEED_STRING, $Key)

                if ($KeyType -eq 'Open_Network') {
                    $preseed.SetNetworkSetting('wlan_keytype', [PreseedOptionType]::PRESEED_STRING, 0)
                } elseif ($KeyType -eq 'WPA/WPA2_PSK') {
                    $preseed.SetNetworkSetting('wlan_keytype', [PreseedOptionType]::PRESEED_STRING, 1)
                } else {
                    $preseed.SetNetworkSetting('wlan_keytype', [PreseedOptionType]::PRESEED_STRING, 2)
                }
            }
