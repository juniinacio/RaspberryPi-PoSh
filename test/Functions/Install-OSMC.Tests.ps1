...
InModuleScope RaspberryPi-PoSh {
    Describe "Install-OSMC" -Tags "CI" {
			BeforeAll {
				...
				$FilePath = Get-ChildItem -Path (Join-Path -Path $Env:HOME -ChildPath 'Downloads/') -Filter "OSMC_TGT_rbp2_*.img.gz" | Sort-Object -Property LastWriteTime -Descending | Select-Object -First 1 -ExpandProperty FullName
